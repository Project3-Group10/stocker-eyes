...
def addStockDB(stock_data):
    ...
    newStock = models.Stock(name = name1, dateDB = date1, open_price = openP, high_price = highP, low_price = lowP, close_price = closeP, adjusted_clase_price = adjustedClaseP, volume_price = volumeP)
    #open_price = openP, 
...
def on_connect():
    ...
    x = models.Stock.query.filter_by(name ='erii').first()
    if x is None: 
        addStockDB(api_data)
