...
def addStockDB(stock_data):
    ...
    for key in stock_data['Time Series (Daily)']:
        ...
        openP = stock_data['Time Series (Daily)'][key]['1. open']
        highP = stock_data['Time Series (Daily)'][key]['2. high']
        lowP = stock_data['Time Series (Daily)'][key]['3. low']
        closeP = stock_data['Time Series (Daily)'][key]['4. close']
        adjustedClaseP = stock_data['Time Series (Daily)'][key]['5. adjusted close']
        volumeP = stock_data['Time Series (Daily)'][key]['6. volume']
        newStock = models.Stock(name = name1, dateDB = key, open_price = openP, high_price = highP, low_price = lowP, close_price = closeP, adjusted_clase_price = adjustedClaseP, volume_price = volumeP)
