...
def addNewUserDB(user_data):
    ...
    newUser = models.User(email = user_data['emailAddress'], name = user_data['name'], avatar = user_data['imageUri'], status = user_data['status'])
    DB.session.add(newUser)
    DB.session.commit()
    #allUsers = models.Person.query.all()

def addStockDB(stock_data):
    name1 = 'er3'
    date1 = '2021-04-16'
    openP = stock_data['Time Series (Daily)'][yesterday]['1. open']
    #highP = stock_data['Time Series (Daily)'][yesterday]['2. high']
    #lowP = stock_data['Time Series (Daily)'][yesterday]['3. low']
    closeP = stock_data['Time Series (Daily)'][yesterday]['4. close']
    #adjustedClaseP = stock_data['Time Series (Daily)'][yesterday]['5. adjusted close']
    #volumeP = stock_data['Time Series (Daily)'][yesterday]['6. volume']
    
    newStock = models.Stock(name = name1, dateDB = date1, open_price = openP, close_price = closeP)
    #open_price = openP, high_price = highP, low_price = lowP, close_price = closeP, adjusted_clase_price = adjustedClaseP, volume_price = volumeP
...
def on_connect():
    ...
    #x = models.Stock.query.filter_by(name ='OVV').first()
    #print(x)
    #if x is None: 
    addStockDB(api_data)
   # else: 
